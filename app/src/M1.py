...
# %%
# convert issue_date to datetime
def tidy_issue_date(df: pd.DataFrame) -> pd.DataFrame:
    df_cpy = df.copy()
    df_cpy['issue_date'] = pd.to_datetime(df_cpy['issue_date'])
    return df_cpy
# fintech_df = tidy_issue_date()

# %% [markdown]
# ### Convert term to integer (months)

# %%
def tidy_term(df: pd.DataFrame) -> pd.DataFrame:
    df_cpy = df.copy()
    df_cpy['term'] = df['term'].astype(str).str.extract(r'(\d+)').astype(int)
    return df_cpy
# fintech_df = tidy_term()
...
def clean_case_sensitivity(df: pd.DataFrame) -> pd.DataFrame:
    ...
    fintech_df_cpy['emp_title'] = df['emp_title'].str.lower().str.strip()
...
def tidy_type(df: pd.DataFrame) -> pd.DataFrame:
    fintech_df_cpy = df.copy()
    fintech_df_cpy['type'] = df['type'].replace({'joint app': 'joint'})
    return fintech_df_cpy
...
# fintech_df = fillna_emp_title(fintech_df)
...
def add_new_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    fintech_df_new['month_number'] = df['issue_date'].dt.month.astype(int)
    fintech_df_new['salary_can_cover'] = annual_inc >= df['loan_amount']
    ...
    fintech_df_new['letter_grade'] = df['grade'].apply(grade_to_letter)
    fintech_df_new['installment_per_month'] = df['loan_amount'] * (df['int_rate'] / 12) *\
                ((1 + (df['int_rate'] / 12)) ** df['term'])\
                / (((1 + (df['int_rate'] / 12)) ** df['term']) - 1)
...
def save_cleaned_dataset_to_csv(df: pd.DataFrame, file_name: str = 'fintech_data_cleaned', data_dir: str = data_dir):
    if len(file_name.split('.')) == 1 or not file_name.endswith('.csv'):
        file_name += '.csv'
    df.to_csv(data_dir + file_name , index=False)

def save_cleaned_dataset_to_parquet(df: pd.DataFrame, file_name: str = 'fintech_data_cleaned', data_dir: str = data_dir):
    if len(file_name.split('.')) == 1 or not file_name.endswith('.parquet'):
        file_name += '.parquet'
    pq.write_table(pa.Table.from_pandas(df), data_dir + file_name)
    
# save_cleaned_dataset_to_csv(fintech_df, 'fintech_data_MET_P2_52_20136_clean.csv', data_dir)
# save_cleaned_dataset_to_parquet(fintech_df, 'fintech_data_MET_P2_52_20136_clean.parquet', data_dir)
