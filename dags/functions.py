...
def extract_cleaned() -> pd.DataFrame:
    ...
    __save_data(cleaned, M1.lookup_table, 'extract_cleaned')
...
def transform() -> pd.DataFrame:
    ...
    transformed = __transformation(df)

    # save the cleaned data & lookup to parquet file
    __save_data(transformed, M1.lookup_table, 'transform')
...
def __clean(df : pd.DataFrame) -> pd.DataFrame:
    ...
    tidy = __tidy_up(df)
    no_outliers = __handling_outliers(tidy)
    imputed = __imputation(no_outliers)
    return imputed


def __handling_outliers(df : pd.DataFrame) -> pd.DataFrame:
    """
...
def __imputation(df : pd.DataFrame) -> pd.DataFrame:
    """
...
def __tidy_up(df : pd.DataFrame) -> pd.DataFrame:
    """
...
def __transformation(df : pd.DataFrame) -> pd.DataFrame:
    """
...
def __save_data(df: pd.DataFrame, lookup: pd.DataFrame, current_step: str):
    if 'clean' in current_step:
